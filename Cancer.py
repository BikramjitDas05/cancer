...
loaded_model = pickle.load(open('biku4.sav', 'rb'))
...
if st.button('Predict', key='predict_button'):
    ...
    input_df = pd.DataFrame({
        'Value': [CLASS, AGE, MENOPAUSE, TUMORSIZE, INVNODES, NODECAPS, DEGMALIG, BREAST, BREASTQUAD]
    }, index=['Class', 'Age', 'Menopause', 'Tumor-size', 'Inv-nodes', 'Node-caps', 'Deg-malig', 'Breast',
              'Breast-quad']).T
    st.table(input_df)
